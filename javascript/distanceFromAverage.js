...
// # For example:
// # on the first day you did something less than the said average (55, meaning -4.2 compared to the average),
// # much more in the second day (95, 35.8 more than the average and so on).

// # The resulting array of differences starting from [55, 95, 62, 36, 48] is thus [4.2, -35.8, -2.8, 23.2, 11.2].

function distancesFromAverage(arr){
  const average = arr.map(parseFloat).reduce((a, b) => a + b, 0) / arr.length;
  const result = arr.map(n => parseFloat((average - parseFloat(n)).toFixed(2)));
